...
|---------------|-------------|-------|-----|
| `text-monefy-green` / `bg-monefy-green` | `--color-green-500` | `#5BAD8F` | Ingresos, FAB income, acciones primarias |
| `text-monefy-green-dark` / `bg-monefy-green-dark` | `--color-green-700` | `#3D9970` | Hover/pressed del verde primario |
| `text-monefy-green-light` / `bg-monefy-green-light` | `--color-green-400` | `#7CC4A4` | Íconos secundarios, chips de filtro |
| `bg-monefy-mint` | `--color-green-50` | `#E8F5EE` | Fondo de `ion-content` en páginas secundarias |
| `text-monefy-red` / `bg-monefy-red` | `--color-red-400` | `#E57373` | Gastos, FAB expense, alertas |
| `text-monefy-red-dark` | `--color-red-500` | `#C62828` | Texto de error crítico |
| `text-monefy-text-primary` | `--color-gray-900` | `#2D2D2D` | Nombres, valores importantes |
| `text-monefy-text-secondary` | `--color-gray-400` | `#8A9A90` | Labels, metadatos, encabezados de sección |
| `border-monefy-border` | `--color-gray-200` | `#C8D8CE` | Cards, separadores, tiles |
...
<div class="grid grid-cols-4 gap-3">
  @for (cat of categories(); track cat.categoryId) {
    ...
    >
      ...
      <span class="text-xs text-center text-monefy-text-primary font-medium leading-tight line-clamp-2">
        {{ cat.name }}
      </span>
    </div>
  }
...
```html
<!-- Tab activo -->
<button class="font-bold text-monefy-green border-b-2 border-monefy-green pb-1">Mes</button>
<!-- Tab inactivo -->
<button class="text-monefy-text-secondary pb-1">Semana</button>
```
...
- Metadatos / fechas: `text-xs text-monefy-text-secondary`
...
|-------------|----------------|-----------|
| Ingreso / positivo | `text-monefy-green` | `text-green-600`, `text-emerald-*` |
| Gasto / negativo | `text-monefy-red` | `text-red-500`, `text-rose-*` |
| Neutral / secundario | `text-monefy-text-secondary` | `text-gray-400` (fuera del DS) |
...
- [ ] Ingresos usan `text-monefy-green` (no `text-green-600`)
- [ ] Gastos usan `text-monefy-red` (no `text-red-500`)
...
- [ ] Metadatos con `text-monefy-text-secondary` (no `text-gray-500`)
...
- [ ] Nunca `text-monefy-text-secondary` sobre `bg-white` para texto informativo crítico
...
```
file.html:42 — [COLOR] Hex arbitrario #5BAD8F — usar text-monefy-green
file.html:78 — [DS] Ingreso con text-green-600 — usar text-monefy-green
