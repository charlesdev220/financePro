...
describe('SheetsApiService', () => {
  let spectator: SpectatorService<SheetsApiService>;
  ...
  it('does not expose initDatabase()', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect((spectator.service as any).initDatabase).toBeUndefined();
  });

  it('does not expose healthCheck()', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect((spectator.service as any).healthCheck).toBeUndefined();
  });

  // REQ-10: getRange sin ETag previo
  describe('getRange() — first call (no cached ETag)', () => {
    it('sends request without If-None-Match header', () => {
      spectator.service.getRange('TRANSACTIONS!A:O').subscribe();
    ...
    it('returns the response body', async () => {
      ...
      const resultPromise = firstValueFrom(spectator.service.getRange('TRANSACTIONS!A:O'));
      const req = httpMock.expectOne(`${BASE}/${encodeURIComponent('TRANSACTIONS!A:O')}`);
      req.flush(mockData);
  ...
  describe('getRange() — ETag caching', () => {
    it('stores ETag from response header', async () => {
      ...
      const firstCall = firstValueFrom(spectator.service.getRange('USERS!A:F'));
      ...
      const secondCall = firstValueFrom(spectator.service.getRange('USERS!A:F'));
    ...
    it('sends If-None-Match on second call to same range', () => {
      ...
      spectator.service.getRange('CATEGORIES!A:I').subscribe();
    ...
    it('returns null when response body is null (304 simulation)', async () => {
      const resultPromise = firstValueFrom(spectator.service.getRange('WALLETS!A:I'));
  ...
  describe('appendRow()', () => {
    it('sends POST to :append with valueInputOption=USER_ENTERED', () => {
      ...
      spectator.service.appendRow('TRANSACTIONS!A:O', values).subscribe();
  ...
  describe('updateRow()', () => {
    it('sends PUT with valueInputOption=USER_ENTERED', () => {
      ...
      spectator.service.updateRow('TRANSACTIONS!A2:O2', values).subscribe();
  ...
  describe('deleteRow()', () => {
    it('sends POST to :clear', () => {
      spectator.service.deleteRow('TRANSACTIONS!A2:O2').subscribe();
